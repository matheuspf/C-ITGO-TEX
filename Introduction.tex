...
The objective is to find the best solution in a search space that satisfies some criteria, expressed in the form of constraints. For minimization, a general nonlinear constrained optimization problem can be defined as follows: \\[-3em]
...
\noindent
where $f(\cdot)$ is the objective function or fitness function, $\bm{x} = (x_1, ..., x_n)$ is a solution vector containing $n$ continuous or discrete values, $g_j(\cdot), \ j = 1, ..., p$, and $h_k(\cdot), \ k = 1, ..., q$, are the inequality and equality constraints respectively. The vectors $\bm{l}$ and $\bm{u}$ define the lower and upper bounds of the solution space. We also define the sum of constraint violation as: \\[-3em]
...
In this work, we develop some modifications over a successful meta-heuristic for continuous optimization, know as Iterative Topographical Global Optimization. We compare the results obtained on eight difficult constrained engineering design problems against several methods, some reporting state of the art results.
