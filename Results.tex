...
As show in Table \ref{tab:SkillMack_3}, the value of the Skillings-Mack statistic is 63.66813949 with an approximate p-value of 1.2469E-07 calculated from the chi-squared distribution with 16 degrees of freedom, which strongly implies that the null hypothesis does not hold true at critical level of $\alpha$ = 0.05 or $\alpha$ = 0.01 based on the chi-squared p-value. That is, there is at least one method that is significantly better than the others. 
...
To prove the superior convergence of C-ITGO, we present one more nonparametric statistical test, the Wilcoxon signed rank test, also described in \cite{Friedman}. This is a more robust approach of pairwise statistical comparison than the previous, that aim to detect significant difference between the means of two competing techniques. The results shown in Table \ref{tab:Wilcoxon} were generated using the R package \textit{wilcoxon.test}, considering C-ITGO against the other algorithms. Empty entries (problems not solved by some method) are treated by default.
...
Given that C-ITGO always had the best MNFEs, its p-values are the lowest in all statistical tests. When comparing with the IAPSO method, the p-value reported by the Wilcoxon test is 0.01606, the highest among all algorithms compared. The Wilcoxon signed test reports a much smaller p-value for all the other methods, almost one order of magnitude smaller than the p-value reported for IAPSO. In all cases the p-value is smaller than 0.05 (and, except for IAPSO, smaller than 0.005), which strongly implies the rejection of the null hypothesis.