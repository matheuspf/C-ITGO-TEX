...
For SRI, we compare C-ITGO results against 6 optimization methods, namely CPSO, LCA, IPSO, SAMP-Jaya and IAPSO, shown in Table \ref{tab:SP1}. The reported results for the SAMP-Jaya method seems to violate the integer constraint at variable three, but it is included so as to show that even in harder situations the C-ITGO algorithm can outperform any competing methods compared here.

The LCA, IPSO, IAPSO and C-ITGO present similar results, all finding the constrained global optimum solution and having very low standard deviation. The SiC-PSO and CPSO methods present inferior results, with higher average fitness. Besides having significant difference between the best from the mean fitness value, the SiC-PSO reports a standard deviation of 0.
...
We compare C-ITGO against the  QPSO, G-QPSO, CB-ABC, IABC-MAL, SAMP-Jaya and IAPSO methods. The statistical results are shown in Table \ref{tab:PV}. It seems that SAMP-Jaya algorithm reports infeasible results, maybe because the integer constraints are not taken into account. Nevertheless, we still use the results reported by SAMP-Jaya to prove the superior convergence of C-ITGO, even in constrained mixed integer problems.

The algorithms CB-ABC, IABC-MAl, IAPSO and C-ITGO are the only who found the integer constrained optimal solution, and also the ones with smaller mean fitness value. All other methods presented relatively poor performance, with much higher NFEs in average.
